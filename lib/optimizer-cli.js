...
function getNameFromDependency(d) {
    var matches = /^(.+).optimizer\.json$/.exec(nodePath.basename(d));

    if (matches) {
        return matches[1];
    }

    if (/optimizer\.json$/.test(d)) {
...
function run(argv) {
    var parser = require('raptor-args').createParser({
            ...
            '--cache-dir': {type: 'string', description: 'Base cache directory (defaults to "CWD/.cache/optimizer")'},
        ...
        .example('Optimize a set of dependencies', '$0 style.less foo.js template.marko')
    ...
    if (typeof config === 'string') {
        ...
        if (config['optimizer']) {
            // This is kind of a hack, but to allow the configuration for the optimizer module
            // to be alongside other configuration, we look for a configuration nested under a "optimizer"
            // property
    ...
    function doRun() {
        var pageOptimizer = optimizer.create(config, configDir || cwd);
                        ...
                        if (optimizedPages.hasOwnProperty(pageName)) {
                            ...
                            if (args.injectInto && args.injectInto.length) {
                                args.injectInto.forEach(function(target) {
                                    ...
                                    targetHtml = injector.inject(targetHtml, optimizedPage, injectOptions);
    ...
    if (args.watch) {
        ...
        watcher.on('modified', function() {
            optimizer.handleWatchedFileChanged();
