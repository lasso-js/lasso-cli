...
function run(argv) {
    var parser = require('raptor-args').createParser({
            ...
            '--html-dir': {type: 'string', description: 'Output directory for JSON files (defaults to "build")'},
            ...
            '--config-cache-key': {
                type: 'string',
                description: 'The configuration cache key'
            }
        })
    ...
    if (typeof config === 'string') {
        config = require.resolve(nodePath.resolve(process.cwd(), config));
        configDir = nodePath.dirname(config);

        if (nodePath.extname(config) === '.js') {
    ...
    if (args.configCacheKey) {
        config.cacheKey = args.configCacheKey;
    }
